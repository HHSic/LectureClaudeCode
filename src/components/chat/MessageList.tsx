...
import { User, Bot, Loader2, FilePlus, FilePen, FileX, FileMinus, FileSearch } from "lucide-react";
import { MarkdownRenderer } from "./MarkdownRenderer";

function getToolDisplay(toolName: string, args: Record<string, unknown> | undefined): { label: string; icon: React.ReactNode } {
  ...
  if (toolName === "str_replace_editor") {
    ...
    switch (command) {
      case "create":
        return { label: `Writing ${fileName ?? "file"}`, icon: <FilePlus className="w-3 h-3" /> };
      case "str_replace":
      case "insert":
        return { label: `Editing ${fileName ?? "file"}`, icon: <FilePen className="w-3 h-3" /> };
      case "undo_edit":
        return { label: `Reverting ${fileName ?? "file"}`, icon: <FileMinus className="w-3 h-3" /> };
      case "view":
        return { label: `Reading ${fileName ?? "file"}`, icon: <FileSearch className="w-3 h-3" /> };
      default:
        return { label: `Editing ${fileName ?? "file"}`, icon: <FilePen className="w-3 h-3" /> };
    }
  }
  ...
  if (toolName === "file_manager") {
    ...
    switch (command) {
      case "rename":
        return { label: `Renaming ${fileName ?? "file"}${newFileName ? ` → ${newFileName}` : ""}`, icon: <FilePen className="w-3 h-3" /> };
      case "delete":
        return { label: `Deleting ${fileName ?? "file"}`, icon: <FileX className="w-3 h-3" /> };
      default:
        return { label: "Managing file", icon: <FilePen className="w-3 h-3" /> };
    }
  }

  return { label: toolName, icon: <FilePen className="w-3 h-3" /> };
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const { label, icon } = getToolDisplay(tool.toolName, tool.args as Record<string, unknown>);
                            return (
